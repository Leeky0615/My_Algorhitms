'''
    ...
    하는 경우, C < 0인 경우는 타임머신으로 시간을 되돌아가는 경우이다. 1번 도시에서 출발해서
    나머지 도시로 가는 가장 빠른 시간을 구하는 프로그램을 작성하시오.
    도시의 개수 : N (1<=N<=500)
    ...
    벨만 포드 최단 경로 알고리즘은 음의 간선이 포함된 상황에서도 사용할 수 있습니다.
      ㅇ 또한 음수 간선의 순환을 감지할 수 있습니다.
      ㅇ 벨만 포드의 기본 시간 복잡도 O(VE)로 다익스트라 알고리즘에 비해 느립니다.
    ...
    만약 음수 간선 순환이 발생하는지 체크하고 싶다면 3번의 과정을 한 번 더 수행합니다.
      ㅇ 이때, 최단 거리 테이블이 갱신된다면 음수 간선 순환이 존재하는 것입니다.
    ...
    벨만 포드 알고리즘
     - 매번 모든 간선을 전부 확인합니다.
        ㅇ 따라서 다익스트라 알고리즘에서의 최저그이 해를 항상 포함합니다.
...
def bf(start):
    ...
    for i in range(n):
        ...
        for j in range(m):
            ...
            if dist[cur] is not INF and dist[next_node] > dist[cur] + cost:
